...
The invite-only product (100-user testing cap) works end to end and the whole engineering track for going public has landed on `main`: AI cost/abuse guardrails on every AI path (#15), self-serve data deletion (#14), a flag-gated `SELF_SERVE_SIGNUP` switch defaulting off (#17), a `/api/health` check + poll-failure alerting (#21), the `googleapis` 144→173 bump that clears the transitive `uuid` vuln (#20), security response headers with a report-only CSP (#24), SEO canonicals/sitemap/robots (#12), and a Limited-Use privacy section (#16). The real blocker is external: Google restricted-scope verification requires a CASA Tier 2 assessment (~$500-$3,000+/yr, recurs annually), and it can't start until Abhinav approves the budget and signs an assessor SOW. Finley's job there is to source 2-3 quotes; Abhinav decides. The launch flip once verification clears is `SELF_SERVE_SIGNUP=true` plus confirming the `AI_*` guardrail envs. Active code branch right now is `fix/tier-ladder-comp`: `TierLadder` now honors an admin-sheet comped tier via `lib/effectiveTier.ts` (commit `5074a90`), and there are uncommitted WIP edits to `LabelsCard`, `ProductTour`, and `auth.ts` on that branch. Next code effort being scoped (not built): a smarter onboarding that captures name + role and auto-picks the label preset, then back-scans the inbox at signup.
...
- `gmail/webhook` — real-time Pub/Sub push receiver (OIDC-verified against `PUBSUB_PUSH_AUDIENCE`; primary labeling path when a watch is live)
- `gmail/poll` — Vercel cron (`*/30 * * * *`) safety-net sweep; calls `history.list` per user, does **not** depend on a live Gmail watch, idempotent via `ClassifiedThread` dedupe + `gmailHistoryId` advance
...
- `labels/back-scan` — powers "Sync inbox" (`force:true` re-classifies recent threads, capped at 25 most-recent threads / ~45s budget); reconciles labels (applies the single best preset label, strips other preset-managed labels) instead of appending
...
**Tests & CI**
- 8 Vitest suites: `lib/{aiGuard,labelPresets,timeSaved,cities,sampleScenarios}.test.ts` + `app/{privacy,terms,support}/page.test.tsx`
...
- [ ] Deeper historical backfill past the 25-thread back-scan cap (optional). — Finley
