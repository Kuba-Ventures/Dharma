      ...
      <DismissibleCard dismissId="milestone-hero-welcome">
        ...
        >
          ...
          <p className="mt-2 max-w-xl text-sm text-white/70">
            ...
            milestones tied to your home city. Confirm your city in Settings to
            personalize them.
          </p>
        </div>
      </DismissibleCard>
  ...
  return (
    <DismissibleCard dismissId={`milestone-hero-next-${next!.id}`}>
      ...
      >
        ...
        <h2 className="mt-3 font-display text-2xl text-white">{next!.title}</h2>
        <p className="mt-2 max-w-xl text-sm text-white/75">
          {next!.description}
          {remaining > 0 && (
            <span className="text-brand-200"> — {formatHours(remaining)} to go.</span>
          )}
        </p>
      </div>
    </DismissibleCard>
